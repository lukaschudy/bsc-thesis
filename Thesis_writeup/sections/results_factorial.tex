...
The joint Wald test on all four interactions ($\chi^2(4) = 21{,}023$, $p<10^{-15}$) decisively rejects the additive null. The frictions interact, and the patterns identified in the cell-mean analysis are not sampling artefacts. As a separate robustness check, Table~\ref{tab:factorial-acrossN} shows that changing the replication count from 250 to 1000 sessions shifts each cell mean by less than $0.025$ in $\Delta$; a visual version of this check is reported in Appendix~\ref{app:replication-stability}.
...
The combined-friction panels (Figure~\ref{fig:offpath-factorial}) show a clear gradient of retaliation strength.
