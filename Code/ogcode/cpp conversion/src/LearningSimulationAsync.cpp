// LearningSimulationAsync.cpp
//
// Friction 3: asynchronous actions (Maskin-Tirole alternating moves).
//
// Design (see design_choices.txt)
// -------------------------------
// * Strict, deterministic alternation with lock-in length T:
//       movingAgent(iter) = ((iter - 1) / T) mod 2 + 1
//   so agent 1 moves for T consecutive periods, agent 2 moves for the
//   next T, and so on.
// * lockIn = 0 is the simultaneous baseline (both move every period).
//   At T = 0 this file produces output bit-for-bit identical to
//   LearningSimulation.cpp: the RNG streams are untouched and the
//   moving-agent override is skipped.
// * The non-mover's "action" is its previous price (p[1][a] before the
//   p-shift). Its Q-table is still updated from the observed joint
//   reward — both agents learn every period, regardless of who moved.
// * Shared state encoding: state = (p1_prev, p2_prev) with
//       s = 1 + (p1-1)*numPrices + (p2-1)
//   (no per-agent state, unlike latency / noise).
// * epsilon decays every period for both agents (one iter == one unit
//   of calendar time for everybody).
    ...
    int lockIn)
{
    ...
    for (int iSession = 1; iSession <= numSessions; ++iSession) {
        ...
        while (true) {
            ...
            // Let the baseline action-selection routine produce candidate
            // actions and decay epsilon for BOTH agents (consistent with
            // design: calendar time advances for everyone).
            LearningSimulation::computePPrime(
                ExplorationParameters, uExploration,
                strategyPrime, state, iIters,
                pPrime, Q, eps);

            // Async override: if lockIn >= 1, overwrite the non-mover's
            // action with its previous price, so only the mover changes.
            if (lockIn >= 1) {
                const int movingAgent =
                    (((iIters - 1) / lockIn) % 2) + 1;     // 1 or 2
                for (int a = 1; a <= numAgents; ++a) {
                    if (a != movingAgent) {
                        pPrime[a] = p[1][a];               // previous price
                    }
                }
            }

            // Shift price history (DepthState-aware, identical to baseline)
            if (DepthState > 1) {
            ...
            // Q-update for every agent (movers AND non-movers)
            for (int iAgent = 1; iAgent <= numAgents; ++iAgent) {
                ...
                const double newq = oldq + alpha[iAgent] * (
                    ...
                    + delta * maxValQLocal[statePrime][iAgent]
                    - oldq);
            ...
            // Convergence check (shared-state, identical to baseline)
    ...
    // -----------------------------------------------------------------------
    // Summary and A_res.txt (identical to baseline)
    // -----------------------------------------------------------------------
